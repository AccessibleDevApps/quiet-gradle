...
Single-file wrappers that run a project's Gradle wrapper with bounded console output: `quiet-gradle.ps1` for Windows (runs `gradlew.bat`) and `quiet-gradle.sh` for Linux (runs `./gradlew`). Built to keep routine Gradle noise out of an AI agent's context window while saving the complete log for later.
...
Requirements: `quiet-gradle.ps1` needs Windows PowerShell 5.1 or newer. `quiet-gradle.sh` needs Linux with bash 4+, GNU `date` and `tail`, and awk (tested with gawk; macOS is not supported). Both take the same options and produce the same output.

## Install

Copy `quiet-gradle.ps1` (or `quiet-gradle.sh`, and `chmod +x` it) next to your project's `gradlew.bat` (or `gradlew`). Add `.agent-logs/` to the project's `.gitignore`.
...
On Linux, the same with `./quiet-gradle.sh`.
...
> When a task requires direct Gradle execution, use `.\quiet-gradle.ps1` (Windows) or `./quiet-gradle.sh` (Linux) instead of invoking `gradlew.bat` / `gradlew` directly. If the quiet failure summary is insufficient, inspect the saved full log rather than rerunning solely to obtain verbose output.
...
- The `.sh` version starts Gradle with only the log as stdout/stderr and closes every other inherited file descriptor, for the same reason.
- Arguments containing `%` are still expanded by `cmd.exe` in the PowerShell version because `gradlew.bat` is a batch file.
